...
o.add_option(
    "--eor",
    dest="eor",
    default="ps_no_halos_nf0.521457_z9.50_useTs0_zetaX-1.0e+00_200_400Mpc_v2",
...
def dL_dth(z):
    ...
    return 1.9 * (1.0 / a.const.arcmin) * ((1 + z) / 10.0) ** 0.2
...
dish_size_in_lambda = dish_size_in_lambda * (
    array["freq"] / 0.150
)  # linear frequency evolution, relative to 150 MHz
first_null = (
    1.22 / dish_size_in_lambda
)  # for an airy disk, even though beam model is Gaussian
bm = 1.13 * (2.35 * (0.45 / dish_size_in_lambda)) ** 2
nchan = opts.nchan
kpls = dk_deta(z) * n.fft.fftfreq(nchan, old_div(B, nchan))

Tsky = 60e3 * (old_div(3e8, (array["freq"] * 1e9))) ** 2.55  # sky temperature in mK
n_lstbins = old_div(opts.n_per_day * 60.0, obs_duration)
...
mk, mpk = old_div(model[:, 0], h), model[:, 1]  # k, Delta^2(k)
...
uv_coverage[old_div(SIZE, 2), old_div(SIZE, 2)] = 0.0
uv_coverage[:, : old_div(SIZE, 2)] = 0.0
uv_coverage[old_div(SIZE, 2) :, old_div(SIZE, 2)] = 0.0
if opts.no_ns:
    uv_coverage[:, old_div(SIZE, 2)] = 0.0
...
for iu, iv in zip(nonzero[1], nonzero[0]):
    u, v = (
        (iu - old_div(SIZE, 2)) * dish_size_in_lambda,
        (iv - old_div(SIZE, 2)) * dish_size_in_lambda,
    )
    umag = n.sqrt(u ** 2 + v ** 2)
    kpr = umag * dk_du(z)
    kprs.append(kpr)
    ...
    if opts.model in ["mod", "pess"]:
        hor = old_div(dk_deta(z) * umag, array["freq"]) + opts.buff
    elif opts.model in ["opt"]:
        hor = (
            dk_deta(z) * (old_div(umag, array["freq"])) * n.sin(old_div(first_null, 2))
        )
    else:
    ...
    for i, kpl in enumerate(kpls):
        ...
        bm_eff = old_div(
            bm ** 2, bm2
        )  # this can obviously be reduced; it isn't for clarity
        scalar = old_div(X2Y(z) * bm_eff * B * k ** 3, (2 * n.pi ** 2))
        Trms = old_div(Tsys, n.sqrt(2 * (B * 1e9) * tot_integration))
        # add errors in inverse quadrature
        sense[kpr][i] += 1.0 / (scalar * Trms ** 2 + delta21) ** 2
        Tsense[kpr][i] += 1.0 / (scalar * Trms ** 2) ** 2

# bin the result in 1D
delta = dk_deta(z) * (1.0 / B)  # default bin size is given by bandwidth
...
for ind, kpr in enumerate(sense.keys()):
    ...
    sense[kpr] = old_div(sense[kpr] ** -0.5, n.sqrt(n_lstbins))
    Tsense[kpr] = old_div(Tsense[kpr] ** -0.5, n.sqrt(n_lstbins))
    for i, kpl in enumerate(kpls):
        k = n.sqrt(kpl ** 2 + kpr ** 2)
        if k > n.max(mk):
            continue
        # add errors in inverse quadrature for further binning
        sense1d[find_nearest(kmag, k)] += 1.0 / sense[kpr][i] ** 2
        Tsense1d[find_nearest(kmag, k)] += 1.0 / Tsense[kpr][i] ** 2

# invert errors and take square root again for final answer
for ind, kbin in enumerate(sense1d):
    sense1d[ind] = kbin ** -0.5
    Tsense1d[ind] = Tsense1d[ind] ** -0.5

# save results to output npz
n.savez(
    "%s_%s_%.3f.npz" % (name, opts.model, array["freq"]),
    ks=kmag,
    errs=sense1d,
    T_errs=Tsense1d,
)
...
err = n.sqrt((1.0 / n.float(X)))
print("total snr = ", old_div(amp, err))
