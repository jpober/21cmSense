...
B = (
    0.008
)  # largest bandwidth allowed by "cosmological evolution", i.e., the maximum line of sight volume over which the universe can be considered co-eval
z = f2z(opts.freq)
...
Tsky = 60e3 * (old_div(3e8, (opts.freq * 1e9))) ** 2.55  # sky temperature in mK
...
cen = old_div(len(kpls), 2)
for kpr in list(Tsense.keys()):
...
for ind, kpr in enumerate(kprs):
    Tsense2d[ind] = old_div(Tsense2d[ind] ** -0.5, n.sqrt(n_lstbins))
...
n.savez(
    "%s_%.3f_2dsense.npz" % (name, opts.freq), kprs=kprs, kpls=kpls, T_errs=Tsense2d
)
...
p.imshow(
    ...
    extent=[kprs[0], kprs[-1], kpls[0], kpls[-1]],
    vmin=1e3,
    vmax=1e4,
)
p.colorbar()

umag = n.arange(0.0, 200.0)
hor = old_div(C.pspec.dk_deta(z) * umag, opts.freq)
