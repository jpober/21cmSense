...
def beamgridder(xcen, ycen, size):
    ...
    cen = old_div(size, 2) - 0.5  # correction for centering
    xcen += cen
    ycen = -1 * ycen + cen
...
# observing time
t_int = 60.0  # how many seconds a single visibility has integrated
cen_jd = 2454600.90911
start_jd = cen_jd - (1.0 / 24) * (old_div((old_div(obs_duration, t_int)), 2))
end_jd = cen_jd + (1.0 / 24) * (old_div((old_div((obs_duration - 1), t_int)), 2))
times = n.arange(start_jd, end_jd, (1.0 / 24 / t_int))
...
bl_len_min = old_div(
    opts.bl_min, (old_div(a.const.c, (ref_fq * 1e11)))
)  # converts meters to lambda
bl_len_max = 0.0
...
print(
    ...
    % (bl_len_max * (old_div(a.const.c, (ref_fq * 1e11))))
)  # 1e11 converts from GHz to cm
if opts.bl_max:
    bl_len_max = old_div(
        opts.bl_max, (old_div(a.const.c, (ref_fq * 1e11)))
    )  # units of wavelength
    print(
...
dim = int(
    n.round(old_div(bl_len_max, dish_size_in_lambda)) * 2 + 1
...
for cnt, uvbin in enumerate(uvbins):
    ...
    for t in times:
        ...
        u, v, w = aa.gen_uvw(i, j, src=obs_zen)
        _beam = beamgridder(
            xcen=old_div(u, dish_size_in_lambda),
            ycen=old_div(v, dish_size_in_lambda),
            size=dim,
        )
        uvplane += nbls * _beam
        uvsum += nbls * _beam
...
print(
    "Saving file as %s_blmin%0.f_blmax%0.f_%.3fGHz_arrayfile.npz"
    % (name, bl_len_min, bl_len_max, opts.freq)
)

n.savez(
    "%s_blmin%0.f_blmax%0.f_%.3fGHz_arrayfile.npz"
    % (name, bl_len_min, bl_len_max, opts.freq),
    uv_coverage=uvsum,
