...
class TestUtils(TestCase):
    ...
    def test_load_ks(self):
        ...
        _, out_k, _ = py21cm.utils.load_noise_files(os.path.join(self.path, test_file))
...
class TestInterp(TestCase):
    ...
    def test_no_freqs(self):
        out = py21cm.utils.noise_interp2d(None, [0.1, 0.2, 0.3], [1, 2, 3])
    ...
    def test_no_noises(self):
        out = py21cm.utils.noise_interp2d(114, [0.1, 0.2, 0.3], None)
        self.assertEqual(0, out)

    def test_one_ks(self):
        with self.assertRaises(ValueError):
            py21cm.utils.noise_interp2d(114, [0.1], [1])

    def test_two_ks(self):
        with self.assertRaises(ValueError):
            py21cm.utils.noise_interp2d(114, [0.1, 0.1], [1, 2])

    def test_two_one_ks(self):
        out = py21cm.utils.noise_interp2d(114, [[0.1], [0.1]], [[1], [2]])
        self.assertEqual(0, out)

    def test_interp_linear(self):
        out_interp = py21cm.utils.noise_interp2d(
