    ...
    def set_ant_params(self, prms):
        ...
        for i, ant in enumerate(self):
            ...
            try:
                ...
                pass
            if ant_changed:
                ant.pos = n.dot(n.linalg.inv(self._eq2zen), top_pos)
            changed |= ant_changed
        return changed
        ...
        for param in prms:
            ...
            if param == "dish_size_in_lambda":
                ...
                self.array_params["obs_duration"] = (
                    60.0 * FWHM / (15.0 * a.const.deg)
                )  # minutes it takes the sky to drift through beam FWHM
            if param == "antpos":
                bl_lens = n.sum(n.array(prms[param]) ** 2, axis=1) ** 0.5
        return self.array_params
...
nside = 7.0  # hex number
L = old_div(1400, a.const.len_ns)
dL = old_div(1212, a.const.len_ns)  # close packed hex
antpos = []
cen_y, cen_z = 0, 0
...
prms = {
    ...
    "dish_size_in_lambda": 7.0,  # in units of wavelengths at 150 MHz = 2 meters; this will also define the observation duration
...
def get_catalog(*args, **kwargs):
    return a.src.get_catalog(*args, **kwargs)
